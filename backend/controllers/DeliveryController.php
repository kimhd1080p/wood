...
//    }
     public function actionNew($id){
       ...
       $model = $this->findModel($id);
//$model->status=1;
        if (Yii::$app->db->createCommand("UPDATE `buy` SET `status`=3 WHERE bid=$id")->execute()) {
            Yii::$app->db->createCommand("UPDATE `products` SET `pstatus`=4 WHERE `pid`=$model->products_id")->execute();
            //Yii::$app->db->createCommand("UPDATE `products` SET `pstatus`=2 WHERE `pid`=$id")->execute();
            Yii::$app->getSession()->setFlash('alert1',[
        'body'=>'แจ้งการจัดส่งแล้ว.....',
        'options'=>['class'=>'alert-success']
     ]);
             return $this->redirect(['index']);
        }else {
