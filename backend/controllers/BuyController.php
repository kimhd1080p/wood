...
//             if($modelde->save()){
//                Yii::$app->db->createCommand("UPDATE `products` SET `pstatus`=1 WHERE `pid`=$model->products_id")->execute();
//                  Yii::$app->getSession()->setFlash('alert1',[
...
//    }
    ...
    public function actionUpdate($id)
    {
        ...
        if ($model->load(Yii::$app->request->post()) && $model->save()) {
            //&&$modelde->load(Yii::$app->request->post()) && $modelde->save()
            return $this->redirect(['view', 'id' => $model->bid]);
        }
...
//$model->status=1;
       if($check2>0){
           Yii::$app->getSession()->setFlash('alert1',[
        'body'=>'สินค้าเกินจำนวนที่มีขาย',
        'options'=>['class'=>'alert-error']
     ]);
             return $this->redirect(Yii::$app->request->referrer ?: Yii::$app->homeUrl);
           
       }
       else if (Yii::$app->db->createCommand("UPDATE `buy` SET `status`=1 WHERE `user_id`=$user_id and bid=$id")->execute()) {
           Yii::$app->db->createCommand("UPDATE `products` SET `pstatus`=2 WHERE `pid`=$model->products_id")->execute();
   ...
   return $this->redirect(['index']);
    ...
    public function actionCbuy($id,$user_id){
        if (Yii::$app->user->isGuest) {
             return $this->redirect(Yii::$app->user->loginUrl);
        }
       $model = $this->findModel($id);
//$model->status=1;
        if (Yii::$app->db->createCommand("UPDATE `buy` SET `status`=0 WHERE `user_id`=$user_id and bid=$id")->execute()) {
            Yii::$app->db->createCommand("UPDATE `products` SET `pstatus`=1 WHERE `pid`=$model->products_id")->execute();
            //Yii::$app->db->createCommand("UPDATE `products` SET `pstatus`=2 WHERE `pid`=$id")->execute();
            Yii::$app->getSession()->setFlash('alert1',[
        'body'=>'ยกเลกการขายเรียบร้อย',
   ...
   return $this->redirect(['transfer/index']);
    ...
    public function actionCtransfer($id){
        if (Yii::$app->user->isGuest) {
             return $this->redirect(Yii::$app->user->loginUrl);
        }
       $model = $this->findModel($id);
...
//$model->status=1;
         if (Yii::$app->db->createCommand("UPDATE `buy` SET `status`=1 WHERE bid=$id")->execute()) {
            Yii::$app->db->createCommand("UPDATE `products` SET `pstatus`=2 WHERE `pid`=$model->products_id")->execute();
            //Yii::$app->db->createCommand("UPDATE `products` SET `pstatus`=2 WHERE `pid`=$id")->execute();
             Yii::$app->db->createCommand("UPDATE `products` SET `qty`=qty+$model->bqty WHERE `pid`=$modelpo->pid")->execute();
            Yii::$app->getSession()->setFlash('alert1',[
        'body'=>'ยกเลิกการโอนเงินเรียบร้อย',
        'options'=>['class'=>'alert-warning']
     ]);
             return $this->redirect(['transfer/index']);
        }else {
   ...
   return $this->redirect(['transfer/index']);
     ...
     public function actionCdeli($id){
         if (Yii::$app->user->isGuest) {
             return $this->redirect(Yii::$app->user->loginUrl);
        }
       $model = $this->findModel($id);
